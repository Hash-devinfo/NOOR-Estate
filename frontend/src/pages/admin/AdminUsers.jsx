import { useEffect, useRef, useState, useMemo } from "react"; // ✅ removed alias
import { adminUsersStyles as s } from "../../assets/dummyStyles";
import { useAuth } from "../../context/AuthContext";
import axios from "axios";
import API_URL from "../../config";
// ✅ removed backend controller import
import {
  HiOutlineFilter,
...
const AdminUsers = () => {
  ...
  const handleBlock = async (id) => {
    try {
      ...
      alert("Operation failed",err); // ✅ alert only takes one argument
    }
  };
      ...
      <div className={s.containerHeader}>
        <div>
          ...
          <p className={s.headerSubtitle}>
            Monitor platform users and access levels {/* ✅ typos fixed */}
          </p>
        </div>
                  ...
                  <tr key={user._id} className={s.tableRow}>
                    ...
                    <td className={s.tdContact}>
                      <div className={s.contactWrapper}>
                        <div className={s.contactEmail}> <HiOutlineMail className="text-primary"/> {" "}
                         {user.email}</div>
                        {user.phone && (
                          <div className={s.contactPhone}>
                            <HiOutlineIdentification color="394a3b8" />{" "}
                            {user.phone}
                          </div>
                        )}
                      </div>
