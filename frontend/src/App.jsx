...
// to scroll to top whenever routr changes

const ScrollToTopOnRouteChange = () => {
  const { pathname } = useLocation();

  (useEffect(() => {
    window.scrollTo({ top: 0, beaviour: "smooth" });
  }),
    [pathname]);
  return null;
};

// floating scroll to top bth

const ScrollTopButton = () => {
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    // FIX: was useEffect(()={ — missing =>
    const handleScroll = () => {
      setVisible(window.scrollY > 300);
    };
    window.addEventListener("scroll", handleScroll); // FIX: was addEventlistner
    handleScroll();
  ...
  const handleClick = () => {
    window.scrollTo({ top: 0, behavior: "smooth" }); // FIX: was behaviour
  };
    ...
    <button
      ...
      className={`fixed bottom-6 right-6 z-50 flex h-12 w-12
        ...
        transition-all duration-300 ${
          // FIX: was translation-all deuration-300
          visible
            ? "scale-100 opacity-100 bg-emerald-500 text-white hover:bg-green-400"
            : "pointer-events-none scale-0 opacity-0"
        }`}
    >
      <FaChevronUp size={22} /> {/* FIX: was FaCheveronUp */}
    </button>
  );
};

// Smart layout for seller and  buyer
...
const App = () => {
  ...
  }, []); // prevent horizontal overflow on whole app
