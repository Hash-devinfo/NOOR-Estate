...
export const addProperty=async(req, res)=>{
    try {
      ...
      images: imageUrls,
      seller: req.user._id, //as seller only create a property
...
export const getSellerDashbord = async (req, res) => {
  try {
    const sellerId = new mongoose.Types.ObjectId(req.user._id); // ✅ proper ObjectId

    const totalProperties = await Property.countDocuments({ seller: sellerId });
    const activeListings = await Property.countDocuments({   // ✅ consistent naming
      seller: sellerId,
      status: "sale",
    });
    ...
    const viewsData = await Property.aggregate([
      ...
      { $group: { _id: null, totalViews: { $sum: "$views" } } }, // ✅ "$views" not "$Vews"
    ]);
    ...
    res.json({
      ...
      stats: {
        totalProperties,
        activeListings,  // ✅ matches frontend
        soldProperties,
        totalInquiries,
        totalViews,
      },
...
export const getPropertyCounts=async(req, res)=>{
    ...
    res.status(500).json({
      success: false,
      message:"internal server error while fetching cunts",
      error: error.message,
    });
  }
}